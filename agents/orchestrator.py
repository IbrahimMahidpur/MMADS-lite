...
Flow:
  ...
  6. Visualization → VIZ_COMPLETE
  7. Publish SESSION_END with full result
...
class RunResult:
    ...
    viz_manifest: dict = field(default_factory=dict)          # ← NEW: chart gallery
    files_created: list[str] = field(default_factory=list)
    errors: list[str] = field(default_factory=list)
    message_trace: list[dict] = field(default_factory=list)
    ...
    def to_dict(self) -> dict:
        return {
            ...
            "viz_manifest":       self.viz_manifest,
            "charts_created":     len(self.viz_manifest.get("charts", [])),
            "errors":             self.errors,
...
class AgentOrchestrator:
    ...
    def _wire_subscriptions(self) -> None:
        ...
        self.bus.subscribe(MessageType.VIZ_COMPLETE,  self._on_viz_complete,  self.AGENT_NAME)
    ...
    def _on_viz_complete(self, msg: AgentMessage) -> None:
        logger.info(
            f"[Orchestrator] Visualization complete — "
            f"{msg.payload.get('chart_count', 0)} charts generated"
        )
    ...
    def run(
        ...
        if run_visualization:
            self._run_visualization(documents, session_id, result)
    ...
    def _run_visualization(
        ...
        """
        Run VisualizationAgent after task execution.
        Publishes VIZ_REQUEST, then VIZ_COMPLETE via the viz agent itself.
        """
        tabular_docs = [
            d for d in documents
            if d.data_type == DataType.TABULAR and d.structured_data is not None
        ...
        if not tabular_docs:
            logger.info("[Orchestrator] No tabular data — skipping visualization")
            return

        logger.info("[Orchestrator] Step 5/5 — Generating visualization gallery")

        primary_df = tabular_docs[0].structured_data

        # Auto-detect target column from automl suggestion
        target_col = None
        ...
        try:
            manifest = viz_agent.generate(
                df=primary_df,
                target_col=target_col,
                task_results=result.task_results,
                stat_report=result.statistical_report,
            )
            result.viz_manifest = manifest.to_dict()

            # Add chart HTML files to files_created
            chart_files = [
                str(self.working_dir / session_id / c["filename"])
                for c in manifest.charts
            ]
            result.files_created.extend(chart_files)
            result.files_created = list(dict.fromkeys(result.files_created))

            logger.info(
                f"[Orchestrator] Visualization complete — "
                f"{len(manifest.charts)} charts for session {session_id}"
            )

        except Exception as e:
            logger.warning(f"[Orchestrator] Visualization failed (non-fatal): {e}")
            result.errors.append(f"Visualization warning: {e}")
    ...
    def _finalize(self, result: RunResult, t_start: float, session_id: str) -> RunResult:
        ...
        self.bus.publish(AgentMessage(
            ...
            payload={
                ...
                "tasks_ok":    succeeded,
                "tasks_total": total,
                "charts":      len(result.viz_manifest.get("charts", [])),
        ...
        self.memory.store(
            content=(
                ...
                f"Charts: {len(result.viz_manifest.get('charts', []))}"
            ),
        ...
        logger.info(
            ...
            f"{succeeded}/{total} tasks | "
            f"{len(result.viz_manifest.get('charts', []))} charts | "
