...
# ── Shared state ───────────────────────────────────────────────────────────
orchestrator = AgentOrchestrator(working_dir=str(OUTPUT_DIR))
memory = AgentMemory()
...
class AnalyseResponse(BaseModel):
    ...
    viz_manifest: dict          # chart gallery — types, filenames, narratives
    charts_created: int         # convenience count
    errors: list[str]
    duration_s: float
...
async def analyse(
    ...
    run_statistical_checks: bool = Form(True),
    run_visualization: bool = Form(True),
    max_tasks: int = Form(6),
):
    """
    Full end-to-end agentic analysis pipeline.

    1. Ingest all uploaded files
    2. Run statistical validation (tabular data)
    3. Generate a hypothesis-driven analysis plan with the Planner agent
    4. Execute each task with the Code Execution agent
    5. Generate a visualization gallery (Plotly charts + LLM narrative)
    6. Return all outputs, files, and a structured result

    - **objective**: Natural language goal, e.g. *"Predict churn and explain key drivers"*
    - **session_id**: Reuse a previous session to build on prior memory
    - **run_visualization**: Generate Plotly chart gallery after task execution (default True)
    - **max_tasks**: Limit the number of tasks executed (default 6)
    """
    if not files:
    ...
    try:
        ...
        result = orchestrator.run(
            ...
            run_statistical_checks=run_statistical_checks,
            run_visualization=run_visualization,
            max_tasks=max_tasks,
        )
        return AnalyseResponse(**result.to_dict())
